...
import { Button, Text, Surface, ActivityIndicator, Card, IconButton } from 'react-native-paper';
...
export default function OCRScanScreen({ navigation }) {
  ...
  const processImage = async () => {
    ...
    try {
      const result = await processImageOCR(image);
      setProcessing(false);
      setOcrResult(result);
    } catch (error) {
      setProcessing(false);
            ...
            {ocrResult.success ? (
                  ...
                  <Card key={index} style={styles.itemCard} mode="outlined">
                    <Card.Content>
                      ...
                      <Text variant="bodyMedium">
                        ...
                        {item.price > 0 && ` | Price: $${item.price.toFixed(2)}`}
                      </Text>
                    </Card.Content>
                  </Card>
                ))}
              ...
              <Surface style={styles.errorSurface} elevation={1}>
                <Text style={styles.errorText}>
                  Failed to extract items. {ocrResult.error || 'Please try with a clearer image.'}
                </Text>
              </Surface>
            )}
          </View>
